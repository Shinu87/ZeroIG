...
class Denoise_2(nn.Module):
    def __init__(self, chan_embed=96):
        ...
        self.act = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.conv1 = nn.Conv2d(3, chan_embed, 3, padding=1)   # changed from 6 to 3 input channels
        self.conv2 = nn.Conv2d(chan_embed, chan_embed, 3, padding=1)
        self.conv3 = nn.Conv2d(chan_embed, 3, 1)              # changed from 6 to 3 output channels
...
class Network(nn.Module):
    ...
    def forward(self, input):
        ...
        # No illumination guidance: only pass reflection to Denoise_2, but keep 6 channels for loss compatibility
        H3_pred_ref = H11.detach() - self.denoise_2(H11)
        H3_pred = torch.cat([H3_pred_ref, torch.zeros_like(H3_pred_ref)], 1)  # [B, 6, H, W]
        H3_pred = torch.clamp(H3_pred, eps, 1)
        H13 = H3_pred[:, :3, :, :]
        s13 = H3_pred[:, 3:, :, :]

        H4_pred_ref = H12.detach() - self.denoise_2(H12)
        H4_pred = torch.cat([H4_pred_ref, torch.zeros_like(H4_pred_ref)], 1)
        H4_pred = torch.clamp(H4_pred, eps, 1)
        H14 = H4_pred[:, :3, :, :]
        s14 = H4_pred[:, 3:, :, :]

        H5_pred_ref = H2.detach() - self.denoise_2(H2)
        H5_pred = torch.cat([H5_pred_ref, torch.zeros_like(H5_pred_ref)], 1)
        ...
        return (L_pred1, L_pred2, L2, s2, s21, s22, H2, H11, H12,
                H13, s13, H14, s14, H3, s3, H3_pred, H4_pred,
...
class Finetunemodel(nn.Module):
    ...
    def forward(self, input):
        ...
        H2 = input / s2
        H2 = torch.clamp(H2, eps, 1)
        # No illumination guidance: only pass reflection to Denoise_2
        H5_pred = H2.detach() - self.denoise_2(H2)
        H5_pred = torch.clamp(H5_pred, eps, 1)
        H3 = H5_pred
        return H2, H3
